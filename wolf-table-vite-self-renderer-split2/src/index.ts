...
export default class Table {
  ...
  cellFunc(func: ( number,  number)=> string | Cell) {
       this.cell = func;
       return this;
  }
  ...
  render() {
    ...
    if (this._other_table) {
        this._other_table.sync_render();
    }
  ...
  sync_render() {
    ...
    _renderer
      //.scrollRows(_data.scroll[0])     // no vertical sync
      //.scrollCols(_data.scroll[1])     // no horizontal sync
      .merges(_data.merges)
      .freeze(_data.freeze)
      .styles(_data.styles)
      .borders(_data.borders)
 ...
 //     resizeContentRect(this);
  ...
  set_sync_table(table) {
      this._other_table = table;
  }
  /**
   * copy data to ...
   * @param to
