...
import { cellRender, cellBorderRender ,cellBorderRender2} from "./cell-render";
...
function renderBorder(
  ...
  } else if (borderLineStyle[0] === "double2") {
    // if type === 'none', you can delete borders in ref(range)
    if (type === "outside" || type === "all") {
      cellBorderRender2(canvas, borderRect, borderLineStyle, "outer",true);
    } else if (type === "left") {
      cellBorderRender2(
        canvas,
        borderRect,
      ...
      cellBorderRender2(canvas, borderRect, { top: borderLineStyle }, "outer",autoAlign);
    } else if (type === "right") {
      cellBorderRender2(
        canvas,
        borderRect,
...
//
        range.eachCol((index) => {
          if (index <= range.endCol) {
            const r1 = range.clone();
            r1.endCol = r1.startCol = index;
            range.eachRow((index2) => {
              if (index2 <= range.endRow) {
                r1.endRow = r1.startRow = index2;
                if (r1.intersects(area.range)) {
                  cellBorderRender2(
                    canvas,
                    area.rect(r1),
