...
//
  ...
  let freeze_decoded = $derived(decode(freeze));
    ...
    if (e.key == 'Enter') {
      ...
      */
      ...
      async function next_input_focus() {
        ...
        let cell_editor = root.querySelector('#cell_editor');
        cell_editor.focus();
        cell_editor.click();
      }
      next_input_focus();
  ...
  export function onTextAreaKeyDown(e, r, c) {
    //console.log("onTextAreaKeyDown:", e.key, r, c)
    ...
    if (e.key == 'Enter' && e.altKey) {
      ...
      async function input_focus() {
        ...
        //cell_editor.dispatchEvent(event); // switch for  textarea
      }
    ...
    if (e.key == 'Enter' && !e.altKey) {
      ...
      */
      ...
      let element = table.rows[parseInt(r2) + 1].cells[parseInt(c2) + 1];
      let event = new MouseEvent('dblclick', { bubbles: true, cancelable: true });
      element.dispatchEvent(event); // switch for  textarea

      console.log("PRE--" +  data[r.i][c.i] + "--");
  ...
  export function onTextAreaChange(value, row, column) {
    //console.log('onTextAreaChange:'+  value + "--");
    cmdz = true;
    if (row.i > data.length - 1) {
      data = [
        ...data,
  ...
  function sum_top_height(r) {
     let height = 0;
     const offset = 28;
     for ( let i = 0 ; i < r ; i++) {
      let tdid = 'td_' + String(i) + '_' + '0';
      //console.log(tdid);
      let ele = root.querySelector('#' + tdid);
      //console.log(ele);
  ...
  function sum_left_width(c) {
     let width = 0;
     const offset = 50;
     for ( let i = 0 ; i < c ; i++) {
      let tdid = 'td_' + '0' + '_' + String(i);
      //console.log(tdid);
      let ele = root.querySelector('#' + tdid);
      //console.log(ele);
      if (ele == null) { return 0;}
  ...
  function isrFreeze() {
      return freeze_decoded.c > 0
  }

  function freezeStyle_head_tr() {
     let style = "";
     if (isrFreeze()) {
        style = "position: sticky;  top: 0px;  background: white; z-index: 20; ";
     } 
     return style;
  }
  ...
  function freezeStyle_head_td(c) {
     ...
     if (isrFreeze()) {
        if ( c == 0 ) {
           style = "position: sticky; left: 50px; width: 100px;  min-width: 100px;  background: white; z-index:18; "
        } else if ( c == 1 ) {
           style = "position: sticky; left: 200px; width: 100px;  min-width: 100px;  background: white; z-index:18; "
        }
     }
  ...
  function freezeStyle_tr(r) {
     let style = "";
     if (isrFreeze()) {
           if ( r < freeze_decoded.r ) {
                let hight = sum_top_height(r)
                if ( hight != 0 ) {
                    //console.log("r", r, hight)
                    style = `color:blue; position: sticky;  top: ${hight}px;  background: white; z-index: 20; `;
                }
           }

/*
           if ( r == 0 ) {
              style = "color:blue; position: sticky;  top: 27px;  background: white; z-index: 20; ";
           } else if ( r == 1 ) {
              style = "color:blue; position: sticky;  top: 52px;  background: white; z-index: 20; ";
           } else if ( r == 2 ) {
              style = "color:blue; position: sticky;  top: 92px;  background: white; z-index: 20; ";
           } 
*/
/*
           if ( r == 0 ) {
              style = "color:blue; position: sticky;  top: 28px;  background: white; z-index: 20; ";
           } else if ( r == 1 ) {
              style = "color:blue; position: sticky;  top: 51px;  background: white; z-index: 20; ";
           } else if ( r == 2 ) {
              style = "color:blue; position: sticky;  top: 91px;  background: white; z-index: 20; ";
           } 
*/
     }
     return style
  ...
  function freezeStyle_td(c,r) {
     let style = "";
     if (isrFreeze()) {
           if ( c < freeze_decoded.c ) {
                let width = sum_left_width(c)
                if ( width != 0) {
                     //console.log("c", c, width)
                     style += `position: sticky; left: ${width}px; width: 100px;  min-width: 100px;  background: white; z-index:18; `
                     if ( c == freeze_decoded.c -1) {
                         style += "border-right: 2px solid green; "
                     }
                }
           }
           if ( r == freeze_decoded.r -1) {
                   style += "border-bottom: 2px solid green; "
           }
/*
        if ( c == 0 ) {
           style += " position: sticky; left: 50px; width: 150px;  min-width: 100px;  background: white; z-index:18; "
        } else if ( c == 1 ) {
           style += "border-right: 2px solid green;  position: sticky; left: 200px; width: 100px;  min-width: 100px;  background: white; z-index:18; "
        } 
*/
/*
        if ( r == 2 ) {
  ...
  */ 
  ...
  onMount(() => {
    if (window && window.document) {
      ...
      //rowElements = root.querySelector('.virtual-row');
      //colElements = root.querySelector('.virtual-col');
      mounted = true;
  ...
  function onMouseDown(e) {
      console.log("onMouseDown",{ c: e.target.dataset.x, r: e.target.dataset.y });
...
*/
  ...
  let { top, left, right, bottom, topLeft, bottomRight,  selectWidth, selectHeight } =  $derived.by(() => {
   ...
   if (mounted) {
      ...
      //console.log("selected id",tlid, brid);

      // tlid  merge cell check

      let tl_ele = root.querySelector('#' + tlid);
      let br_ele = root.querySelector('#' + brid);
      //let tr = root.querySelector('#tr_0' );
      //console.log("tl_ele",tl_ele);
      //console.log("br_ele",br_ele);
      ...
      if (tl_ele != null && br_ele != null) {
        ...
        top = tl_ele.offsetTop ;
        left = tl_ele.offsetLeft;
        right = left + tl_ele.clientWidth + tl_ele.clientLeft ;
        bottom = top +  tl_ele.clientHeight + tl_ele.clientTop ;
        selectWidth = right - left ;
        selectHeight = bottom - top ;
  ...
  function shift(x, r) {
   if (r.i == 3 && x.i == 6 ) { return 7 }
   if (r.i == 3 && x.i == 7 ) { return 8 }
   if (r.i == 3 && x.i == 8 ) { return 9 }

   if (r.i == 4 && x.i == 5 ) { return 7 }
   if (r.i == 4 && x.i == 6 ) { return 8 }
   if (r.i == 4 && x.i == 7 ) { return 9 }
   if (r.i == 4 && x.i == 8 ) { return 10 }

   if (r.i == 5 && x.i == 5 ) { return 7 }
   if (r.i == 5 && x.i == 6 ) { return 8 }
   if (r.i == 5 && x.i == 7 ) { return 9 }
   if (r.i == 5 && x.i == 8 ) { return 10 }
   return x.i
 }

  function mergeSkip(x, r) {
      //console.log(mergeMap);
/*
      let data_merged=GetColSpan(mergeCells, x.i, r.i) || GetRowSpan(mergeCells, x.i, r.i)
      let colspan=GetColSpan(mergeCells, x.i, r.i)
      let rowspan=GetRowSpan(mergeCells, x.i, r.i)

      if (data_merged) {
             console.log("merged", x.i,  r.i, encode({ c:x.i , r: r.i }, colspan, rowspan));
             console.log( colspan, rowspan);

      }
     //return false;
*/
     //return mergeMap[r.i, x.i] == 1
     let f = mergeMap[r.i][ x.i] == 1
     //if (f) { console.log("skip",r.i, x.i) };
     return f;
  }
...
*/
  ...
  $effect(() => {
    if (mounted) {
      ...
      if (tl_ele != null && br_ele != null && !edition) {
      ...
      } else if  (tl_ele != null && br_ele == null && !edition) {

        let top = tl_ele.offsetTop ;
        let left = tl_ele.offsetLeft;
        let right = left + tl_ele.clientWidth + tl_ele.clientLeft ;
        let bottom = top +  tl_ele.clientHeight + tl_ele.clientTop ;
                   ...
                   >
                     {#if String(edition) == String([x.i, r.i])}
                       <textarea
                         ...
                         onkeydown={(e) => onTextAreaKeyDown(e, r, x)}
